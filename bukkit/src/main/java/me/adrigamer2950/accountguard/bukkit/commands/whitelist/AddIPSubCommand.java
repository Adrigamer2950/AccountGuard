...
public class AddIPSubCommand extends SubCommand<AGBukkit> {
    ...
    public boolean execute(User user, String label, String[] args) {
        ...
        if (getPlugin().addIP(player.getUniqueId(), ip))
            user.sendMessage(getPlugin().config.prefix() + getPlugin().messages.IP_ADDED_IN_WHITELIST().replaceAll("%player%", Objects.requireNonNull(player.getName())));
        else
            user.sendMessage(getPlugin().config.prefix() + getPlugin().messages.IP_ALREADY_IN_WHITELIST().replaceAll("%player%", Objects.requireNonNull(player.getName())));
